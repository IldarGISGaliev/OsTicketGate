...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
   await update.message.reply_text('Привет! Здесь можно подать жалобу, если вы считаете, что подверглись дискриминации по причине вашего гражданства или узнать какие-то полезные факты. Мы свяжемся с вами для уточнения деталей и вместе подумаем, что можно сделать.Перед подачей жалобы, ознакомьтесь с [базой знаний](http://34.34.22.214/upload/index.php), возможно ваш случай уже имеет готовое решение. Введите в поиск название учреждения (банк, авикомпания и т.д) или страну и проверьте результаты.', parse_mode='Markdown',reply_markup=ReplyKeyboardRemove())
